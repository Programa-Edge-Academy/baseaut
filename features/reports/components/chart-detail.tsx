...
/**
 * Performance level labels for session results.
 */
export type PerformanceLevel = "Desenvolver" | "Adequado" | "Satisfatório";
/**
 * Help level labels grouped by type and intensity.
 */
...
/**
 * Exercise result within a session.
 */
...
/**
 * Session detail model used in chart views.
 */
...
/**
 * Props for the chart detail panel.
 */
...
export function ChartDetail({ session, mode }: ChartDetailProps) {
  ...
  /**
   * Maps help levels to theme color classes.
   */
  ...
  return (
    <View className="w-[200px] px-5 py-3.5 bg-level2 rounded-tr-2xl rounded-bl-2xl rounded-br-2xl shadow-panelShadow outline outline-1 outline-offset-[-1px] outline-outline flex flex-col justify-start items-start gap-3.5">
      <Text className="text-muted text-default-3">{session.date}</Text>
      ...
      {visibleExercises.map((ex) => (
        ...
        >
          <Text className="text-white text-default-1">{ex.name}</Text>

          {mode === "performance" && ex.performanceLevel && (
            <Text
              className={`${getPerformanceColor(ex.performanceLevel)} text-default-2`}
            >
              {ex.performanceLevel}
            </Text>
          )}
          ...
          {mode === "help" &&
            ex.helpLevels &&
            ex.helpLevels.length > 0 &&
            ex.helpLevels.map((help, index) => (
              <Text
                key={index}
                className={`${getHelpColor(help)} text-default-2`}
              >
                {help}
              </Text>
            ))}
