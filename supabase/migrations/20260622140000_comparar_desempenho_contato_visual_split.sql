-- ════════════════════════════════════════════════════════════════════
-- rpc_comparar_desempenho_periodos: separar contato visual (pessoas/objetos)
-- ════════════════════════════════════════════════════════════════════
-- Recria a função substituindo o nó "contato_visual" por dois nós distintos,
-- "contato_visual_pessoas" e "contato_visual_objetos", derivados da coluna
-- comportamentos_sessao.observacao. O restante é idêntico à versão anterior.
-- ════════════════════════════════════════════════════════════════════
  ...
  SELECT jsonb_build_object(
    ...
    'comportamentos', (
      SELECT jsonb_build_object(
        ...
        'contato_visual_pessoas', jsonb_build_object(
          'p1', COUNT(*) FILTER (WHERE is_p1 AND tipo = 'contato_visual' AND lower(observacao) LIKE '%pessoa%'),
          'p2', COUNT(*) FILTER (WHERE is_p2 AND tipo = 'contato_visual' AND lower(observacao) LIKE '%pessoa%'),
          'diferenca_absoluta', ABS(
            COUNT(*) FILTER (WHERE is_p2 AND tipo = 'contato_visual' AND lower(observacao) LIKE '%pessoa%')
            - COUNT(*) FILTER (WHERE is_p1 AND tipo = 'contato_visual' AND lower(observacao) LIKE '%pessoa%')
          )
        ),
        'contato_visual_objetos', jsonb_build_object(
          'p1', COUNT(*) FILTER (WHERE is_p1 AND tipo = 'contato_visual' AND lower(observacao) LIKE '%objeto%'),
          'p2', COUNT(*) FILTER (WHERE is_p2 AND tipo = 'contato_visual' AND lower(observacao) LIKE '%objeto%'),
          'diferenca_absoluta', ABS(
            COUNT(*) FILTER (WHERE is_p2 AND tipo = 'contato_visual' AND lower(observacao) LIKE '%objeto%')
            - COUNT(*) FILTER (WHERE is_p1 AND tipo = 'contato_visual' AND lower(observacao) LIKE '%objeto%')
          )
        ),
